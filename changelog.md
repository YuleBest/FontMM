...
- **新增「固定行距 & 字距」**（高级配置，可开关；issue #17）：应用字体时把装入系统的字体行距**统一**到同一档位，并收掉虚高的上下留白，换字体后行距不再跟着变
  - 档位三选：紧凑 1.20× / 标准 1.45×（默认）/ 宽松 1.60×；只改度量字段，字形与 `FONTS/` 里的原始字体不动
  - 起因：安卓的行高取「paint 度量」与「该行实际使用字体度量」的**并集**（只能抬高不能压低），而本模块配置里 `sans-serif` 最前面正是英文槽位，所以换英文字体连中文行距都会变；另外字体的 `head` 包围盒虚高时，每段上下会多出一大圈空白（实测某字体多出约 1.4 倍字身框）
  - 字体集合（`.ttc` / `.otc`）同样支持；改写后会回读自检，不通过就保留原字体
  - 已知限制：模块自带的生僻字补充字库（藏文、盲文等）不参与统一，含这些字形的行仍会更高
...
- **模块更新后保留用户设置**：字重覆写模式、自定义字重映射、固定行距开关与档位此前会在更新时丢失，现在随 `FONTS/` 一起继承
