...
var W = (() => {
  ...
  return (n) => {
    for (
      var i = n.length,
        s = new Uint8Array((((i - (n[i - 1] == '=') - (n[i - 2] == '=')) * 3) / 4) | 0),
        r = 0,
        a = 0;
      r < i;
    ) {
...
for (v = 0; v < N.length; v++)
  ((p.prototype['get' + P(N[v])] = D(N[v])),
    (p.prototype['set' + P(N[v])] = (function (e) {
      return function (t) {
        this[e] = !!t;
      };
    })(N[v])));
...
for (b = 0; b < I.length; b++)
  ((p.prototype['get' + P(I[b])] = D(I[b])),
    (p.prototype['set' + P(I[b])] = (function (e) {
      return function (t) {
...
for (E = 0; E < S.length; E++)
  ((p.prototype['get' + P(S[E])] = D(S[E])),
    (p.prototype['set' + P(S[E])] = (function (e) {
      return function (t) {
        this[e] = String(t);
      };
    })(S[E])));
...
function oe(e) {
  let t =
    ...
    n = e.IN_NODE && !t,
    i = !e.IN_NODE && !e.IN_DENO && !e.IN_BUN,
    s =
      i &&
      typeof window < 'u' &&
...
async function C() {
  if (
    !f.IN_NODE ||
    (($ = (await import('node:url')).default),
    (B = await import('node:fs')),
    (L = await import('node:fs/promises')),
    (se = (await import('node:vm')).default),
    (x = await import('node:path')),
    (T = x.sep),
    typeof R < 'u')
  )
    return;
  let e = B,
    ...
    n = await import('ws'),
    i = await import('node:child_process'),
    s = { fs: e, crypto: t, ws: n, child_process: i };
...
function de(e, t) {
  return (
    ...
    e.includes('://')
      ? { response: fetch(e) }
      : { binary: L.readFile(e).then((n) => new Uint8Array(n.buffer, n.byteOffset, n.byteLength)) }
  );
}
...
async function me(e) {
  return (
    e.startsWith('file://') && (e = e.slice(7)),
    e.includes('://') ? await import(e) : await import($.pathToFileURL(e).href)
  );
}
o(me, 'nodeLoadScript');
async function H(e) {
    ...
    s = {
      ...
      getLocalSet: o((r) => {
        ...
        for (; d.length;) {
          let u = d.pop(),
            y = t.stat(u);
          (t.isDir(y.mode) && d.push.apply(d, t.readdir(u).filter(c).map(l(u))),
            (a[u] = { timestamp: y.mtime, mode: y.mode }));
        }
        return { type: 'local', entries: a };
      }, 'getLocalSet'),
      getRemoteSet: o(async (r) => {
        let a = Object.create(null),
          c = await pe(r.opts.fileSystemHandle);
        for (let [l, d] of c)
          l !== '.' &&
            (a[i.join2(r.mountpoint, l)] = {
              timestamp:
      ...
      storeRemoteEntry: o(async (r, a, c) => {
        let l = r.get(i.dirname(a)),
          d = t.isFile(c.mode)
            ? await l.getFileHandle(i.basename(a), { create: !0 })
            : await l.getDirectoryHandle(i.basename(a), { create: !0 });
        if (d.kind === 'file') {
      ...
      reconcile: o(async (r, a, c) => {
        ...
        (Object.keys(a.entries).forEach(function (m) {
          let g = a.entries[m],
            h = c.entries[m];
          (!h || (t.isFile(g.mode) && g.timestamp.getTime() > h.timestamp.getTime())) &&
            (d.push(m), l++);
        }),
          d.sort());
        let u = [];
        ...
        for (let m of u)
          if (c.type === 'local') s.removeLocalEntry(m);
          else {
            let g = i.normalize(m.replace(r.mountpoint, '/')).substring(1);
...
function he(e) {
  ...
  return async (n) => {
    ...
    (n.FS.mkdirTree('/lib'),
      (n.API.sitePackages = `/lib/python${i}.${s}/site-packages`),
...
function Ne(e) {
  ...
  return (
    ...
    [he(t), ve(e.env.HOME), be(e.env), z, ...Ee(e.fsInit)]
  );
}
o(Ne, 'getFileSystemInitializationFuncs');
function Ie(e) {
  if (typeof WasmOffsetConverter < 'u') return;
  let { binary: t, response: n } = _(e + 'pyodide.asm.wasm'),
    i = K();
  return function (s, r) {
    return (
      (async function () {
...
var U = Y;
async function Se(e = {}) {
  if ((await C(), e.lockFileContents && e.lockFileURL))
    throw new Error("Can't pass both lockFileContents and lockFileURL");
  let t = e.indexURL || (await J());
  if (
    ((t = k(w(t))),
    (e.packageBaseUrl = k(e.packageBaseUrl)),
    (e.cdnUrl = k(e.packageBaseUrl ?? `https://cdn.jsdelivr.net/pyodide/v${U}/full/`)),
    !e.lockFileContents)
  ) {
...
async function ke(e, t) {
  ...
  let n = await e._loadSnapshot,
    i = ArrayBuffer.isView(n) ? n : new Uint8Array(n);
...
function Fe(e, t) {
  ...
  if (
    ...
    n.version !== U && t.checkAPIVersion)
  )
    throw new Error(
      `Pyodide version does not match: '${U}' <==> '${n.version}'. If you updated the Pyodide version, make sure you also updated the 'indexURL' parameter passed to loadPyodide.`,
    );
  e.locateFile = (i) => {
    throw i.endsWith('.so')
      ? new Error(`Failed to find dynamic library "${i}"`)
      : new Error(`Unexpected call to locateFile("${i}")`);
  };
}
o(Fe, 'configureAPI');
function Ae(e, t, n) {
