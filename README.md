...
开启后，模块会把系统自带字体**挖空**成一个只有度量、没有文字字形的载体，插到各家族最前面当基准：行距固定在系统标准上，换字体不再影响它，空格宽度也随之统一。

- 载体只有 2KB 上下；文字仍然由你选择的字体渲染，它只提供行距与空格宽度
...
A：行距由字体自身的度量决定，换字体就会跟着变。开启「高级配置 → 固定行距 & 字距」后会以系统自带的度量为基准，换字体不再改变行距。
