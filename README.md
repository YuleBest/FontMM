...
安卓的行高由「字体家族里与请求字重最接近的那个字体」与「每行实际用到的字体」共同决定，而且**只能抬高不能压低** —— 本模块的配置里排在 `sans-serif` 最前面的正是「英文 & 数字」槽位，所以换英文字体时，中文行距也会跟着变。

开启后模块会做两件事，让行距只由你选的档位决定，不再随字体变化：

- 把装入系统的字体（中英日韩等槽位字体、等宽、Emoji）的行距度量**按比例统一**到同一档位（字体上下的比例保持不变，字形完全不动；`FONTS/` 里的原始字体不动）
- 同时收紧字体的「包围盒」——它决定段落**上下那圈空白**的大小。有些字体这个值虚高，会让每段多出一大圈留白，一并收掉
- 额外把一个只含度量、没有文字字形的内置载体放到各家族最前面，让系统读取的那一侧也一致

档位（按字身框倍数）：

| 档位 | 行距 | 说明 |
| ---- | ---- | ---- |
| **紧凑** | 1.20× | 比原字体行距更紧，一屏能多显示一行左右 |
| **标准** | 1.45× | 默认，接近系统中文字体的行距 |
| **宽松** | 1.60× | 明显更透气 |

- 默认关闭；切换开关或档位后需要重新点一次「应用」才生效
- 模块自带的生僻字补充字库（藏文、盲文等图形较高的字体）不参与统一，含这些字形的行会更高
...
A：行距由字体自身的度量决定，换字体就会跟着变。开启「高级配置 → 固定行距 & 字距」后，装入字体的行距会被统一到你选的档位，换字体不再改变行距。
