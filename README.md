...
安卓的行高由「字体家族里与请求字重最接近的那个字体」与「每行实际用到的字体」共同决定 —— 本模块的配置里排在 `sans-serif` 最前面的正是「英文 & 数字」槽位，所以**换英文字体时，中文行距也会跟着变**。

开启后，模块会把内置的度量字体（由 Roboto Flex 裁出，只有度量、没有文字字形）插到各家族最前面当基准：行距固定在标准值上，换字体不再影响它，空格宽度也随之统一。

- 载体只有十几 KB；文字仍然由你选择的字体渲染，它只提供行距与空格宽度
- 默认关闭；切换后需要重新点一次「应用」才生效
- 它只能把行距**钉在一个下限**：若所选字体本身的行距比系统标准还大，那一行仍以该字体为准
...
A：行距由字体自身的度量决定，换字体就会跟着变。开启「高级配置 → 固定行距 & 字距」后会以模块内置的标准度量为基准，换字体不再改变行距。
